...
class TestInventoryEndpoints:
    ...
    def test_get_inventory_filter_by_body_style(self):
        response = client.get("/api/inventory?body_style=Truck")
        data = response.json()
        assert response.status_code == 200
    ...
    def test_get_inventory_filter_by_fuel_type(self):
        response = client.get("/api/inventory?fuel_type=Electric")
        data = response.json()
        assert response.status_code == 200
        for vehicle in data["vehicles"]:
    ...
    def test_get_inventory_price_range(self):
        response = client.get("/api/inventory?min_price=40000&max_price=60000")
        data = response.json()
        assert response.status_code == 200
        for vehicle in data["vehicles"]:
    ...
    def test_get_featured_vehicles(self):
        response = client.get("/api/inventory/featured")
        assert response.status_code == 200
        data = response.json()
    ...
    def test_get_available_models(self):
        response = client.get("/api/inventory/models")
        assert response.status_code == 200
        data = response.json()
        assert "models" in data

    def test_search_inventory(self):
        response = client.get("/api/inventory/search?q=Silverado")
        assert response.status_code == 200
        data = response.json()
        assert "vehicles" in data
    ...
    def test_get_vehicle_by_id_not_found(self):
        response = client.get("/api/inventory/v999999")
        assert response.status_code == 404

    def test_get_vehicle_by_vin_not_found(self):
        response = client.get("/api/inventory/vin/INVALIDVIN123")
        assert response.status_code == 404
...
class TestRecommendationEndpoints:
    ...
    def test_recommendations_vehicle_not_found(self):
        response = client.get("/api/recommendations/v999999")
        assert response.status_code == 404
